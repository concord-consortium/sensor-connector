...
Sensor Connector can be launched on Windows with command line arguments to turn on file logging.  On Mac systems, file logging is always turned on.  When file logging is enabled, a Sensor Connector log file is created that stores information which includes errors and exceptions, memory usage statistics, and a record of which internal functions were called.  When Sensor Connector is run from a command line on Windows, use the `-l` or `-log` argument to turn on log files.  Additionally, use the `xml` argument to switch from simple logging to XML formatted file logging and use the `severe` argument to set the log level to only log severe information such as errors and exceptions.
